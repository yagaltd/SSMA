...
#[tokio::test]
async fn audio_sessions_command_and_stream_events_are_ephemeral() -> Result<()> {
    let (gateway_base, gateway_handle) = spawn_gateway(String::new(), false).await?;
    let client = reqwest::Client::new();

    let create = client
        .post(format!("http://{}/audio/sessions", gateway_base))
        .json(&json!({
    ...
    let anon_cookie = extract_cookie(&create, "ssma_anon").expect("anonymous cookie");
    let create_json = create.json::<Value>().await?;
    let session_id = create_json["session"]["audioSessionId"]
        .as_str()
        .expect("audio session id")
        .to_string();
    let channel = create_json["session"]["channel"]
        .as_str()
        .expect("audio channel")
        .to_string();
    assert!(create_json["session"]["rtcSessionId"].is_string());
    ...
    let start = client
        .post(format!(
            "http://{}/audio/sessions/{}/commands",
            gateway_base, session_id
        ))
        .header("Cookie", &anon_cookie)
        .json(&json!({
            "command": "start",
            "payload": {"voiceActivity": true}
        }))
        .send()
        .await?;
    assert_eq!(start.status(), reqwest::StatusCode::OK);
    let start_json = start.json::<Value>().await?;
    assert_eq!(start_json["session"]["status"], "streaming");

    let invalidate = ws_wait_for(&mut ws, "channel.invalidate").await?;
    assert_eq!(invalidate["channel"], channel);
    assert_eq!(invalidate["intents"][0]["intent"], "audio.session.started");
    ...
    let get_session = client
        ...
        .send()
        .await?;
    assert_eq!(get_session.status(), reqwest::StatusCode::OK);
    let get_json = get_session.json::<Value>().await?;
    assert_eq!(get_json["session"]["status"], "streaming");
    ...
    assert_eq!(
        replay_after["intents"]
            ...
            .unwrap_or(0),
        0,
        "audio session events must stay out of durable replay"
    );
    replay_ws
        .send(Message::Text(
            json!({ "type": "channel.subscribe", "channel": create_json["session"]["channel"], "params": {} }).to_string(),
    ...
    let replay_snapshot = ws_wait_for(&mut replay_ws, "channel.snapshot").await?;
    assert_eq!(
        replay_snapshot["intents"][0]["intent"],
...
#[tokio::test]
async fn backend_audio_events_fan_out_on_audio_channels() -> Result<()> {
    let (gateway_base, gateway_handle) = spawn_gateway(String::new(), false).await?;
    let client = reqwest::Client::new();

    let create = client
        .post(format!("http://{}/audio/sessions", gateway_base))
        .json(&json!({ "mode": "transcription" }))
        .send()
        .await?;
    assert_eq!(create.status(), reqwest::StatusCode::CREATED);
    let anon_cookie = extract_cookie(&create, "ssma_anon").expect("anonymous cookie");
    ...
    let pushed = client
        ...
        .send()
        .await?;
    assert_eq!(pushed.status(), reqwest::StatusCode::ACCEPTED);

    let invalidate = ws_wait_for(&mut ws, "channel.invalidate").await?;
    assert_eq!(invalidate["channel"], channel);
    assert_eq!(
        invalidate["intents"][0]["intent"],
        "audio.session.partial_transcript"
    );
    assert_eq!(
        invalidate["intents"][0]["payload"]["payload"]["text"],
        "hello world"
    );

    let mut replay_ws = connect_with_cookie(ws_url, Some(&anon_cookie)).await?;
    let _ = ws_wait_for(&mut replay_ws, "hello").await?;
    let _ = ws_wait_for(&mut replay_ws, "replay").await?;
    replay_ws
        .send(Message::Text(
            json!({ "type": "channel.subscribe", "channel": create_json["session"]["channel"], "params": {} }).to_string(),
        ))
